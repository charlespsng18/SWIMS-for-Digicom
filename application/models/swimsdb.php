<?php
    ...
    class swimsdb extends CI_Model {
		...
		public function get_suppliers_details($supplier_name){
            return $this->db->from("suppliers")->where("supplier_name", $supplier_name)->get()->result_array();
        }
		...
		public function get_all_product_of_supplier($username){
			$sql ="select * from products p
					join pending_supplies ps on p.product_id = ps.product_id
                    join suppliers s on ps.supplier_id = s.supplier_id
				where s.supplier_name = '{$username}';" ;
			return $this->db->query($sql)->result_array();
		}
		public function get_prod_low(){   //changed
			$sql ="select * from products p
					join supplies s on p.product_id = s.product_id
                    join suppliers ps on ps.supplier_id = s.supplier_id
				where p.quantity_in_stock <= p.reorder_level";
			return $this->db->query($sql)->result_array();
		}
		public function add_pending_supply($product_id, $quantity, $supplier_id){
			$sql = "INSERT INTO pending_supplies 
			(product_id, supplier_id, quantity) 
			VALUES ('{$product_id}','{$supplier_id}', '{$quantity}')";
			if ($this -> db -> query($sql))
				return true;
			else
				return false;
		}
		public function confirm_transfer_product($product_name, $supplier_name){  //changed
			$sql = "select * from products p
					join pending_supplies ps on p.product_id = ps.product_id
                    join suppliers s on ps.supplier_id = s.supplier_id
				where product_name='{$product_name}' 
					and supplier_name='{$supplier_name}'";
			$confirm_detail = $this -> db -> query ($sql) -> result_array();
			$only_confirm_detail= $confirm_detail[0];
			$quantity_to_add=(int)$only_confirm_detail["quantity"];
			$product_id=$only_confirm_detail["product_id"];
			$supplier_id=$only_confirm_detail["supplier_id"];
			
			$sql = "update products 
				set quantity_in_stock = quantity_in_stock + {$quantity_to_add}
				where product_id = {$product_id};";
			$this -> db -> query($sql);			
			$sql = "delete from pending_supplies
